...
# when every pending change is documentation (*.md). Docs are low-risk content, so
# a session report or a doc tweak may land straight on main without the branch+PR
# ceremony. Code still requires a branch.
#
# The rule is the EXTENSION, never the directory: docs/ also holds docs.spec.ts
# and architecture.spec.ts, and a fitness function is code by any measure.
#
...
echo "Exception: a commit touching only *.md files may go directly to main." >&2
exit 2
